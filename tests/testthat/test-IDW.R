testthat::test_that("IDW interpolation works correctly", {
  ...
  testthat::expect_true(inherits(Interpolated_data$Ensamble, "SpatRaster"))
  testthat::expect_equal(terra::nlyr(Interpolated_data$Ensamble), length(unique(BD_Obs$Date)))
  ##################################################################################################
  ...
  # Check that the result is a raster object
  testthat::expect_true(inherits(Interpolated_SN, "SpatRaster"))
  testthat::expect_equal(terra::nlyr(Interpolated_SN), length(unique(BD_Obs$Date)))
  ##################################################################################################
  ...
  Interpolated_SnRain <- IDW(BD_Obs, BD_Coord, shapefile, grid_resolution = 5, p = 2,
                           ...
                           stat_validation = "M004", save_model = FALSE, name_save = NULL)
  # Check that the result is a raster object
  testthat::expect_true(inherits(Interpolated_SnRain$Ensamble, "SpatRaster"))
  testthat::expect_equal(terra::nlyr(Interpolated_SnRain$Ensamble), length(unique(BD_Obs$Date)))
  ##################################################################################################
})
